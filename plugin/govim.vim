...
function s:install(force)
  ...
  if a:force || $GOVIM_ALWAYS_INSTALL == "true" || !filereadable(targetdir."govim") || !filereadable(targetdir."gopls")
    ...
    " TODO: make work on Windows
    let install = system("env GO111MODULE=on CGO_ENABLED=1 GOBIN=".shellescape(targetdir)." go install github.com/govim/govim/cmd/govim golang.org/x/tools/gopls 2>&1")
    if v:shell_error
      throw install
    endif
  endif
